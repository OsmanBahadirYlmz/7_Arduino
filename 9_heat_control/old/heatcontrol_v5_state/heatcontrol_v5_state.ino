...
if(T2<(T1+1) & T2>(T1-1))
{state=0;}
else if(T2<T1) //sıcaklık düşüyor
{state=1;}
else if (T2>T1) // sıcaklık yükseliyor
{state=2;}
...
{
  if (T0<(SetPoint-1))//below setpoint
  {switch (state)
    {
     case 0:
     pos=pos+step1;
  ...
  else if (T0>(SetPoint+1)) //upper setpoinr
  {switch (state)
    {
     case 0:
     pos=pos-step1;
